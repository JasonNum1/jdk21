...
大多数的并发集合实现（包括大多数的队列）和常用的java.util有所不同，在并发集合中，Iterators和Spliterators提供了弱一致性而不是快速失败迭代。
    1.他们会使用其他的操作并发的进行
    2.他们绝不会抛出并发修改异常
    3.他们保证遍历在构造时已经存在的元素，并且只遍历一次，并且会（但不保证）反应构造之后的任何吸怪
这里需要说明下弱一致性和快速失败迭代：
...
synchronized和volatile指令，和Thread.start()，Thread.join()一样可以形成happens-before关系，特别是：
    ...
    5.一个线程的所有操作都发生在改线程的join方法成功返回之前。
在java.util.concurrent包和它的子包内所有类的方法都继承了这些保证到高级别的同步。
特别是：
    1.在一个线程内向并发集合内放置元素的行为发生在另一个线程访问或移除那个元素的行为。
    2.在一个线程内Runnable的提交到执行器发生在它的执行开始之前。类似于Callables提交到ExecutorService。
    3.一个线程的Future的异步计算行为发生在另个线程的Future.get()方法之前
    4.优先于释放同步器方法的行为，例如Lock.unlock, Semaphore.release, and CountDownLatch.countDown，发生在后续成功获取的行为，例如Lock.lock, Semaphore.acquire, Condition.await，并且在另一个线程内CountDownLatch.await在同一个同步器对象上。
...
```
#### [java语言规范](https://docs.oracle.com/javase/specs/jls/se21/html/jls-17.html#jls-17.4.5)
```text
介绍happens-before顺序
```
